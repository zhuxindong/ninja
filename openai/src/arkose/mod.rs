...
impl ArkoseToken {
    ...
    /// Get Platform ArkoseLabs token
    #[inline]
    pub async fn new_platform() -> anyhow::Result<Self> {
        ...
        get_platform_arkose_token_from_bx(&bx_json.to_string()).await
    }

    /// Get ChatGPT ArkoseLabs token from bx
    #[inline]
    pub async fn new_from_bx(bx: &str) -> anyhow::Result<Self> {
        get_arkose_token_from_bx(bx).await
    }

    /// Get Platform ArkoseLabs token from bx
    #[inline]
    pub async fn new_platform_from_bx(bx: &str) -> anyhow::Result<Self> {
        get_platform_arkose_token_from_bx(bx).await
    }

    /// Get ArkoseLabs token from context (Only support ChatGPT)
    #[inline]
    ...
    /// Get ArkoseLabs token from HAR file (Support ChatGPT and Platform)
    #[inline]
...
async fn get_arkose_token_from_bx(bx: &str) -> anyhow::Result<ArkoseToken> {
    get_arkose_token_from_bx_common(
...
async fn get_platform_arkose_token_from_bx(bx: &str) -> anyhow::Result<ArkoseToken> {
    get_arkose_token_from_bx_common(
...
async fn get_arkose_token_from_bx_common(
    host: &str,
    bx: &str,
    public_key: &str,
    site: &str,
    ...
    let hanlde = move || async { Ok(arkose_token) };

    if Context::get_instance().await.arkose_solver().is_some() {
        submit_if_invalid(hanlde).await
    } else {
        hanlde().await
    }
}
...
async fn get_arkose_token_from_context() -> anyhow::Result<ArkoseToken> {
    ...
    if ctx.arkose_solver().is_some() {
        let token = submit_if_invalid(|| ArkoseToken::new()).await?;
        return Ok(token);
    }
